...
class MPU9250Async {
    ...
    void writeByteAsync(uint8_t address, uint8_t subAddress, uint8_t data, i2c_async::I2CWriteByteCallback callback, uint16_t delay_ms=0) {
        i2c_async::I2CAsyncOperation::async_write_single_register(_i2c, address, subAddress, data, callback, delay_ms);
    }

    template<std::size_t SIZE>
    void writeBytesAsync(uint8_t address, const std::array<std::tuple<uint8_t, uint8_t, uint8_t>, SIZE>& series, i2c_async::I2CWriteByteCallback callback) {
        i2c_async::I2CAsyncOperation::async_write_multiple_registers(_i2c, address, series, callback);
    }

    void readByteAsync(uint8_t address, uint8_t subAddress, i2c_async::I2CReadByteCallback callback, uint16_t delay_ms=0) {
        i2c_async::I2CAsyncOperation::async_read_single_register(_i2c, address, subAddress, callback, delay_ms);
    }

    void readBytesAsync(uint8_t address, uint8_t subAddress, uint8_t count, i2c_async::I2CReadBytesCallback callback, uint16_t delay_ms=0) {
        i2c_async::I2CAsyncOperation::async_read_multiple_registers(_i2c, address, subAddress, count, callback, delay_ms);
    }

public:
    MPU9250Async(mbed::drivers::v2::I2C* i2c, uint8_t busId): _i2c(i2c), _busId(busId) {
        getAres();
        getGres();
        getMres();
        _timer.start();
    ...
    mbed::drivers::v2::I2C* getI2C(void) {
      return _i2c;
    }

    void initAllAsync(MPU9250SimpleCallback finalCallback) {
        if (_initialized) {
            finalCallback(i2c_async::OK);
            return;
        }
    ...
    void whoAmI1(i2c_async::I2CReadByteCallback callback) {
        readByteAsync(MPU9250_ADDRESS, WHO_AM_I_MPU9250, callback);  // Read WHO_AM_I register for MPU-9250
    }

    void whoAmI2(i2c_async::I2CReadByteCallback callback) {
        readByteAsync(AK8963_ADDRESS, WHO_AM_I_AK8963, callback);  // Read WHO_AM_I register for AK-8963
    }
    ...
    void resetMPU9250Async(MPU9250SimpleCallback callback) {
        // reset device
...
private:
    ...
    void constructTotalAccelBiases(MPU9250SimpleCallback callback, Calibratioin* context) {
        // Construct the accelerometer biases for push to the hardware accelerometer bias registers. These registers contain
        // factory trim values which must be added to the calculated accelerometer biases; on boot up these registers will hold
        // non-zero values. In addition, bit 0 of the lower byte must be preserved since it is used for temperature
        // compensation calculations. Accelerometer bias registers expect bias input as 2048 LSB per g, so that
        // the accelerometer biases calculated above must be divided by 8.
        readBytesAsync(MPU9250_ADDRESS, XA_OFFSET_H, 2, [this, callback, context](i2c_async::StatusCode status, std::size_t, uint8_t* values) {
            if (status != i2c_async::OK) {
                callback(status);
                return;
            }
            context->accel_bias_reg[0] = (int16_t) ((int16_t) values[0] << 8) | values[1];
            delete[] values;

            readBytesAsync(MPU9250_ADDRESS, YA_OFFSET_H, 2, [this, callback, context](i2c_async::StatusCode status, std::size_t, uint8_t* values) {
                if (status != i2c_async::OK) {
                    callback(status);
                    return;
                }
                context->accel_bias_reg[1] = (int16_t) ((int16_t) values[0] << 8) | values[1];
                delete[] values;

                readBytesAsync(MPU9250_ADDRESS, ZA_OFFSET_H, 2, [this, callback, context](i2c_async::StatusCode status, std::size_t, uint8_t* values) {
                    if (status != i2c_async::OK) {
                        callback(status);
                        return;
                    }
                    context->accel_bias_reg[2] = (int16_t) ((int16_t) values[0] << 8) | values[1];
                    delete[] values;
                    ...
                    for(uint8_t ii = 0; ii < 3; ii++) {
                        if(context->accel_bias_reg[ii] & mask) mask_bit[ii] = 0x01; // If temperature compensation bit is set, record that fact in mask_bit
                    }

                    // Construct total accelerometer bias, including calculated average accelerometer bias from above
                    context->accel_bias_reg[0] -= (context->accel_bias[0]/8); // Subtract calculated averaged accelerometer bias scaled to 2048 LSB/g (16 g full scale)
                    context->accel_bias_reg[1] -= (context->accel_bias[1]/8);
                    context->accel_bias_reg[2] -= (context->accel_bias[2]/8);

                    uint8_t data[6];
                    data[0] = (context->accel_bias_reg[0] >> 8) & 0xFF;
                    data[1] = (context->accel_bias_reg[0])      & 0xFF;
                    data[1] = data[1] | mask_bit[0]; // preserve temperature compensation bit when writing back to accelerometer bias registers
                    data[2] = (context->accel_bias_reg[1] >> 8) & 0xFF;
                    data[3] = (context->accel_bias_reg[1])      & 0xFF;
                    data[3] = data[3] | mask_bit[1]; // preserve temperature compensation bit when writing back to accelerometer bias registers
                    data[4] = (context->accel_bias_reg[2] >> 8) & 0xFF;
                    data[5] = (context->accel_bias_reg[2])      & 0xFF;
                    ...
                    */
                    ...
                    float * dest1 = _accelBias;
                    dest1[0] = (float) context->accel_bias[0]/(float) context->accelsensitivity;
                    dest1[1] = (float) context->accel_bias[1]/(float) context->accelsensitivity;
                    dest1[2] = (float) context->accel_bias[2]/(float) context->accelsensitivity;

                    printf("AccelBias X: %f\r\n", dest1[0]);
                    printf("AccelBias Y: %f\r\n", dest1[1]);
                    printf("AccelBias Z: %f\r\n", dest1[2]);

                    callback(i2c_async::OK); // finish!
                });
            });
        }); // Read factory accelerometer trim values
    }

    void enterAccelGyroBiasesComputationLoop(MPU9250SimpleCallback callback, Calibratioin* context) {
        if (context->ii < context->packet_count) {
            ++context->ii;
            readBytesAsync(MPU9250_ADDRESS, FIFO_R_W, 12, [this, callback, context](i2c_async::StatusCode status, std::size_t, uint8_t* values) {
                if (status != i2c_async::OK) {
                    callback(status);
                    return;
                }
                int16_t accel_temp[3] = {0, 0, 0}, gyro_temp[3] = {0, 0, 0};
                accel_temp[0] = (int16_t) (((int16_t) values[0] << 8) | values[1]);    // Form signed 16-bit integer for each sample in FIFO
                accel_temp[1] = (int16_t) (((int16_t) values[2] << 8) | values[3]);
                accel_temp[2] = (int16_t) (((int16_t) values[4] << 8) | values[5]);
                gyro_temp[0]  = (int16_t) (((int16_t) values[6] << 8) | values[7]);
                gyro_temp[1]  = (int16_t) (((int16_t) values[8] << 8) | values[9]);
                gyro_temp[2]  = (int16_t) (((int16_t) values[10] << 8) | values[11]);

                context->accel_bias[0] += (int32_t) accel_temp[0]; // Sum individual signed 16-bit biases to get accumulated signed 32-bit biases
                context->accel_bias[1] += (int32_t) accel_temp[1];
                context->accel_bias[2] += (int32_t) accel_temp[2];
                context->gyro_bias[0]  += (int32_t) gyro_temp[0];
                context->gyro_bias[1]  += (int32_t) gyro_temp[1];
                context->gyro_bias[2]  += (int32_t) gyro_temp[2];

                delete[] values;
                enterAccelGyroBiasesComputationLoop(callback, context);
            });
            return;
        }

        context->accel_bias[0] /= (int32_t) context->packet_count; // Normalize sums to get average count biases
        context->accel_bias[1] /= (int32_t) context->packet_count;
        context->accel_bias[2] /= (int32_t) context->packet_count;
        context->gyro_bias[0]  /= (int32_t) context->packet_count;
        context->gyro_bias[1]  /= (int32_t) context->packet_count;
        context->gyro_bias[2]  /= (int32_t) context->packet_count;

        if (context->accel_bias[2] > 0L) {
            context->accel_bias[2] -= (int32_t) context->accelsensitivity;
        ...
        /*
            uint8_t data[6];
            data[0] = (-context->gyro_bias[0]/4    >> 8) & 0xFF; // Divide by 4 to get 32.9 LSB per deg/s to conform to expected bias input format
            data[1] = (-context->gyro_bias[0]/4)         & 0xFF; // Biases are additive, so change sign on calculated average gyro biases
            data[2] = (-context->gyro_bias[1]/4    >> 8) & 0xFF;
            data[3] = (-context->gyro_bias[1]/4)         & 0xFF;
            data[4] = (-context->gyro_bias[2]/4    >> 8) & 0xFF;
            data[5] = (-context->gyro_bias[2]/4)         & 0xFF;
        */
        ...
        */
        ...
        dest1[0] = (float) context->gyro_bias[0]/(float) context->gyrosensitivity; // construct gyro bias in deg/s for later manual subtraction
        dest1[1] = (float) context->gyro_bias[1]/(float) context->gyrosensitivity;
        dest1[2] = (float) context->gyro_bias[2]/(float) context->gyrosensitivity;

        printf("GyroBias X: %f\r\n", dest1[0]);
        printf("GyroBias Y: %f\r\n", dest1[1]);
        printf("GyroBias Z: %f\r\n", dest1[2]);

        // Skip the followiing function call as the computed values are never used so far
        // constructTotalAccelBiases(callback, context);
        float * dest2 = _accelBias;
        dest2[0] = (float) context->accel_bias[0]/(float) context->accelsensitivity;
        dest2[1] = (float) context->accel_bias[1]/(float) context->accelsensitivity;
        dest2[2] = (float) context->accel_bias[2]/(float) context->accelsensitivity;
...
public:
    ...
    void accelgyrocalMPU9250Async(MPU9250SimpleCallback callback) {
        //                                 register, data, delay in ms
        static const std::array<std::tuple<uint8_t, uint8_t, uint8_t>, 16> ACCEL_GYRO_CAL_0 = {
            // reset device, reset all registers, clear gyro and accelerometer bias registers
            std::make_tuple(PWR_MGMT_1, 0x80, 100), // Write a one to bit 7 reset bit; toggle reset device
            // get stable time source
            // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
            std::make_tuple(PWR_MGMT_1, 0x01, 0),
            std::make_tuple(PWR_MGMT_2, 0x00, 200),
            // Configure device for bias calculation
            std::make_tuple(INT_ENABLE, 0x00, 0),   // Disable all interrupts
            std::make_tuple(FIFO_EN, 0x00, 0),      // Disable FIFO
            std::make_tuple(PWR_MGMT_1, 0x00, 0),   // Turn on internal clock source
            std::make_tuple(I2C_MST_CTRL, 0x00, 0), // Disable I2C master
            std::make_tuple(USER_CTRL, 0x00, 0),    // Disable FIFO and I2C master modes
            std::make_tuple(USER_CTRL, 0x0C, 15),   // Reset FIFO and DMP
            // Configure MPU9250 gyro and accelerometer for bias calculation
            std::make_tuple(CONFIG, 0x01, 0),       // Set low-pass filter to 188 Hz
            std::make_tuple(SMPLRT_DIV, 0x00, 0),   // Set sample rate to 1 kHz
            std::make_tuple(GYRO_CONFIG, 0x00, 0),  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
            std::make_tuple(ACCEL_CONFIG, 0x00, 0), // Set accelerometer full-scale to 2 g, maximum sensitivity
            // Configure FIFO to capture accelerometer and gyro data for bias calculation
            std::make_tuple(USER_CTRL, 0x40, 0),    // Enable FIFO
            std::make_tuple(FIFO_EN, 0x78, 40),     // Enable gyro and accelerometer sensors for FIFO (max size 512 bytes in MPU-9250)
                                                    // accumulate 40 samples in 80 milliseconds = 480 bytes
            std::make_tuple(FIFO_EN, 0x00, 10),     // Disable gyro and accelerometer sensors for FIFO
                                        // ^^ This delay is required for the sensor to store the sampling results
        };
        writeBytesAsync(MPU9250_ADDRESS, ACCEL_GYRO_CAL_0, [this, callback](i2c_async::StatusCode status) {
            if (status != i2c_async::OK) {
                callback(status);
                return;
            }
            Calibratioin* context = new Calibratioin(); // delete in the final callback
            MPU9250SimpleCallback wrapper = [this, callback, context](i2c_async::StatusCode status) {
                if (context) {
                    delete context;
                }
                callback(status);
            ...
            readBytesAsync(MPU9250_ADDRESS, FIFO_COUNTH, 2, [this, wrapper, context](i2c_async::StatusCode status, std::size_t, uint8_t* values) {
                ...
                context->fifo_count = ((uint16_t) values[0] << 8) | values[1];
                context->packet_count = context->fifo_count/12;  // How many sets of full gyro and accelerometer data for averaging
                enterAccelGyroBiasesComputationLoop(wrapper, context);
            });
        });
    }
